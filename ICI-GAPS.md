...
전체 17건 중 **7건 수정됨**. ici 는 v0.5.4 까지 왔다.
...
| **C-1** | **순수 C++ 프로젝트는 기본 설정으로 초록불 불가** | 🔴 미해결 |
| **C-9** | **C++ 테스트 CWD 가 엔진마다 다름** | 🔴 미해결 |
| C-2 | 스위트 상태와 콘솔 카운트 모순 | 미해결 |
| C-3 | test 엔진이 실패 사유를 안 남김 | 미해결 |
| C-5 | C++ 에서 `type` 이 항상 WARN | 부분 완화 (#65) |
| C-7 | ESTIMATED 가 SKIP 보다 관대 | 미해결 |
| C-8 | AST 폴백이 검사 대상 미보고 | 미해결 |
| A-2 / A-3 / B-2 / B-3 | 빌드 어댑터·moc·basename 해석·언어 지원 | 미해결 |

남은 것 중 **C-1 과 C-9 가 가장 무겁다.** 전자는 게이트를 쓸 수 없게 만들고,
후자는 같은 코드가 gcov 설치 여부에 따라 통과하거나 실패하게 만든다.
