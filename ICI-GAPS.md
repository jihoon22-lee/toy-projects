...
전체 17건 중 **11건 수정됨**. ici 는 v0.5.5 까지 왔다.
...
| C-5 | C++ 에서 `type` 이 항상 WARN | ✅ #65 (탭) + #68 (N/A) |
| C-7 | ESTIMATED 가 SKIP 보다 관대 | 상당 부분 해소 (#68) |
| C-8 | AST 폴백이 검사 대상 미보고 | 미해결 |
| A-2 / A-3 / B-2 / B-3 | 빌드 어댑터·moc·basename 해석·언어 지원 | 미해결 |

**무겁다고 표시했던 C-1 과 C-9 가 닫혔다.** 그 결과 이 저장소의 프로젝트들이
`[engines.dead] required = false` 우회 없이 `suite=PASS` 에 도달한다.

남은 것은 대부분 Qt/빌드시스템 지원(A-2/A-3)이며, 이는 애초에 Phase 5 로 계획된 별도 작업이다.
