...
`feat/buildscope-config-diff`. B0 established the producer/consumer boundary:
...
B4 adds semantic comparison of two raw compile databases. The local implementation and native
contract tests are present on the feature branch; B4 PR/remote evidence and the 0.5.0 release remain
pending while the B5 release boundary is open.
...
configuration diff implementation/local evidence is recorded below, while hybrid release integration
(B5) remains pending and the ici I3 target-by-target comparison is complete.
...
B4 configuration diff is the current `0.5.0` feature-branch candidate.
...
The checksum-validated public ici v0.9.0 release also passes an uncached deep verification with
suite `WARN`: 14 engines = 11 PASS / 3 WARN / 0 FAIL / 0 ERROR / 0 SKIP, `92/92` tests,
line/function/branch coverage `93.5% / 99.0% / 76.7%`, sanitizer PASS, compile DB `12/12`
production units and `27` configurations, and TEM `4.95/5.0`. The standalone Zero-CDN HTML is
1,235,505 bytes, SHA-256
`0c98a38b27e928df2c60dcadff9ecc3daa1072cb620354d9f4a9fe8d9b987f80`, with title
`ici Verification Report — buildscope`. PR/remote CI, hosted report publication, and release
...
release_root="$(mktemp -d /tmp/buildscope-ici-v0.9.0.XXXXXX)"
...
# ici 0.9.0 public release verification. Provision this interpreter outside the repo
...
above; B4 implementation/local evidence is recorded above, B5 hybrid release integration remains
pending, and the ici I3 target-by-target comparison is complete.
...
local evidence are recorded above; B5 hybrid release integration remains future work, while the ici I3
target-by-target same-basename comparison is complete.
