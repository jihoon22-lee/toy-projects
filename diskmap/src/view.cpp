...
CandidateEvaluation evaluateSizePredicate(const FsNode& node,
                                          const ViewFilter& filter) {
    const std::optional<std::uint64_t> lower = minimumSize(filter);
    const std::optional<std::uint64_t> upper = maximumSize(filter);
    if (!lower.has_value() && !upper.has_value()) {
        return CandidateEvaluation{true, true};
    }
    const MetricValue value = metricValue(node, filterMetric(filter),
                                          filter.scanner_totals_filtered);
    if (!value.known) {
    ...
    const bool below = lower.has_value() && value.bytes < *lower;
    const bool above = upper.has_value() && value.bytes > *upper;
    return CandidateEvaluation{!below && !above, true};
}

CandidateEvaluation evaluateAgePredicate(const FsNode& node,
                                         const ViewFilter& filter) {
    const std::optional<std::int64_t> lower = modifiedAfter(filter);
    const std::optional<std::int64_t> upper = modifiedBefore(filter);
    if (!lower.has_value() && !upper.has_value()) {
        return CandidateEvaluation{true, true};
    }
    const FsMetadata* metadata = metadataForAge(node);
    ...
    const bool below = lower.has_value() && metadata->modified_ns < *lower;
    const bool above = upper.has_value() && metadata->modified_ns > *upper;
    return CandidateEvaluation{!below && !above, true};
}
...
CandidateEvaluation evaluateLargestCandidate(const FsNode& node,
    ...
    const CandidateEvaluation size = evaluateSizePredicate(node, filter);
    if (!size.matches) {
        return size;
    }
    const CandidateEvaluation age = evaluateAgePredicate(node, filter);
    if (!age.matches) {
        return age;
    }
...
bool inspectLargestNode(const FsNode& node,
                        std::size_t limit,
                        const ViewFilter& filter,
                        const SortSpec& sort,
    ...
    if (issue != NodeIssue::None) {
        ...
        return node.is_dir && issue == NodeIssue::ScannerFiltered;
    }
    if (node.is_dir) {
        return true;
    }
    if (!isRegularFileEntry(node)) {
        return false;
    }
...
LargestFilesResult largestFiles(const FsNode& node,
    ...
    while (!stack.empty()) {
        ...
        if (!frame.inspected) {
            ...
            if (!inspectLargestNode(current, limit, filter, sort, result)) {
                stack.pop_back();
                continue;
            }
        }
