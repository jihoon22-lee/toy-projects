...
**문서 기준일:** 2026-09-01. 이 계획은 toy-projects [PR #12](https://github.com/jihoon22-lee/toy-projects/pull/12)로 `main`에 병합됐다. 현재 완료 상태는 이 문서의 체크리스트와 병합된 PR을 함께 기준으로 삼는다.
...
기록한다. B4 configuration diff implementation/local evidence가 현재 feature branch에서
완료됐고, B4 PR/remote evidence와 B5 hybrid release integration은 미완료이며, ici I3
target-by-target same-basename 비교는 완료됐다.
...
configuration diff implementation/local evidence는 아래에 기록하고, B4 PR/remote와 B5 hybrid
release integration은 아직 pending이며, ici I3 target-by-target same-basename comparison은 완료됐다.
...
완료됐으며 B4 configuration diff는 현재 `0.5.0` feature-branch candidate다.

### B4. configuration diff (implementation complete; 0.5.0 candidate, unreleased)

**브랜치:** `feat/buildscope-config-diff`

**현재 상태:** B4 implementation은 현재 feature branch의 `0.5.0` candidate에 반영됐다. 입력은
두 개의 raw `compile_commands.json`만 받고, snapshot v1/v2/v3는 기존 producer compatibility
경계로 유지한다. 출력은 strict `buildscope.diff/v1`이며 Python producer, C++ parser/model, Qt
GUI와 hybrid contract가 같은 의미론을 사용한다.
...
native extension은 없다. checksum-validated public ici v0.9.0 uncached deep suite는 `WARN`,
14 engines = 11 PASS / 3 WARN / 0 FAIL / 0 ERROR / 0 SKIP, `92/92` tests,
line/function/branch `93.5% / 99.0% / 76.7%`, sanitizer PASS, compile DB `12/12`
production units와 `27` configurations, TEM `4.95/5.0`이다. Zero-CDN HTML은 1,235,505
bytes, SHA-256 `0c98a38b27e928df2c60dcadff9ecc3daa1072cb620354d9f4a9fe8d9b987f80`이다.
B4 PR/remote CI, hosted report, release-artifact publication은 TODO/evidence pending이다.

### B5. hybrid integration과 release (B4 이후 대기)

**선행 조건:** B3/B4 implementation과 ici I3 compile-context 기능이 public release로 제공되어야
한다. B4는 implementation/local evidence까지 닫혔지만 PR/remote evidence가 pending이므로
B5 hybrid integration과 제품 release는 아직 시작하지 않았다. clang-tidy/clazy deep-profile
완료 조건은 ici I4의 tool-backed analyzer가 검증·릴리스된 뒤에 닫는다.
...
이 순서는 계획 수립 당시의 역사적 실행 순서다. 현재는 B0~B4 implementation/local evidence가
완료됐고 B4 PR/remote evidence와 B5 release boundary는 아직 열려 있다. 2026-09-01 기준
BuildScope의 다음 경계는 B4 evidence와 B5 configuration-diff hybrid release이며, ici I3
